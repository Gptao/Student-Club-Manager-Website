...
<!-- 再检查有没有查到成员信息  只显示本部门的 -->
<c:if test="${empty requestScope.managerMembers}">
	<jsp:forward page="Member_managerSearch"></jsp:forward>
</c:if>
			...
			<div class="panel">
				...
				<div class="panel-body">
					<table class="table table-striped">
						...
						<c:forEach items="${requestScope.managerMembers}" var="member">
							<tr>
								...
								<td><a href="Member_searchBeforeUpdate?memberId=${member.memberId }">修改</a></td>
                                <td><a href="Member_delete?memberId=${member.memberId }">删除</a></td>
							</tr>
						</c:forEach>
					</table>
					<%-- 每页${requestScope.membersPage.pageSize}条记录 
					...
					<c:forEach begin="1" end="${requestScope.membersPage.totalPage}" step="1" var="i">
						...
						<c:if test="${i!=requestScope.membersPage.currentPage}">
							<a href="Member_managerSearch?currentPage=${i}">${i}</a>
						</c:if>
					</c:forEach>
					页 --%>
				</div>
